...
- **Base de datos**: Supabase (PostgreSQL) con Auth. Proyecto real: **`Silbatazo_v2`**, ref **`llilwqlqgbronvbsffav`**, región `us-east-1`. Los admins tienen cuenta de Supabase Auth por **correo** (rol `admin` en `profiles`). Los veedores entran por **usuario** (no correo — muchos no tienen), con contraseña que por defecto es su teléfono; por dentro sigue siendo Supabase Auth con un correo sintético `usuario@veedores.silbatazo.local` que nunca se le muestra a nadie (ver `VEEDOR_EMAIL_DOMAIN` en `api/create-user.js` y `assets/js/supa.js` — deben coincidir). No hay acceso por link/token para ninguno de los dos roles.
